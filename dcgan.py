    ...
    def discriminate(self, input_images, t_txt, is_train=True):
        ...
        with tf.variable_scope("discriminator", reuse=tf.AUTO_REUSE):
            ...
            if t_txt is not None:
                ...
                net_txt = TileLayer(net_txt, [1, 16, 16, 1], name='d_txt/tile')
...
class Cgan:
    ...
    def build(self, dataset, z_size=100):
        ...
        D_fake, D_logit_fake = self.discriminator.discriminate(self.sample, self.y)
        ...
        with tf.variable_scope('', reuse=tf.AUTO_REUSE):
            ...
            self.D_loss = self.lambda_ * self.d_losses - tf.reduce_mean(tf.log(D_real+1e-6) + tf.log(1. - D_fake+1e-6))
            self.G_loss = self.lambda_ * self.g_losses - tf.reduce_mean(tf.log(D_fake+1e-6))

            D_optimizer = tf.train.AdamOptimizer(learning_rate=1e-6)
            ...
            G_optimizer = tf.train.AdamOptimizer(learning_rate=1e-6)
    ...
    def run(self, n_epochs=5000, output=None, every=100):
        sess = tf.Session()
