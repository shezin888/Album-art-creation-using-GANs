...
class Cgan:
    ...
    def build(self, dataset, z_size=100):
        ...
        self.D_loss = -tf.reduce_mean(tf.log(D_real+1e-6) + tf.log(1. - D_fake+1e-6))
        self.G_loss = -tf.reduce_mean(tf.log(D_fake+1e-6))

        # d_vars = tl.layers.get_variables_with_name('discriminator', True, True)
        # g_vars = tl.layers.get_variables_with_name('generator', True, True)

        #learning rate=0.001 || 1e-6
        self.D_solver = tf.train.AdamOptimizer(learning_rate=1e-6) \
            .minimize(self.D_loss, var_list=self.discriminator.theta_D)
        self.G_solver = tf.train.AdamOptimizer(learning_rate=1e-6) \
